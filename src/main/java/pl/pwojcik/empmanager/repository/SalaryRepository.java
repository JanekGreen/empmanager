...
public interface SalaryRepository extends PagingAndSortingRepository<Salary, Long>{
	...
	@Query(value="select date_format(date, '%M %Y') as header, date , AVG(amount) as average from salaries group by date_format(date, '%M %Y') HAVING date >=?1 and date <= ?2 order by date", nativeQuery=true)
